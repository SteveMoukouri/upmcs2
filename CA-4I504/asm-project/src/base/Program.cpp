...
/* Delimite les fonctions du programme et crée la liste des fonctions 
 Il faut parcourir les lignes composant le programme et chercher les directives de début et de fin d'une fonction (voir TD). Celles-ci delimitent les lignes du programme correspondant à une fonction : il faut alors créer un objet de type Function* et l'ajouter à la liste des fonctions du programme.

NB1 : on peut comparer un string s à une chaine de caractère avec la methode compare :  s.compare(0, 4, "test") rend 0 si le string s contient la chaine test en préfixe (pour les 4 premiers caractères)

NB2 : Etant donnée un pointeur sur ligne l = l-> isDirective() rend true si l est une direction et getDirective(l) rend la Directive * associée.
   La methode get_content de la classe Line (ou Instruction ou Directive) rend le contenu de la ligne (ou directive ou instruction) sous forme de string.
...
void  Program::comput_CFG(){
   ...
   //_myCFG = malloc(sizeof(*_myCFG) * size);

   for(int i=0; i < size; i++) {
     current = get_function(i);
     current->comput_basic_block();
     _myCFG.push_back(new Cfg(current->get_BB(0), current->nbr_BB()));
   }
